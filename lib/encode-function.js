...
const bySchema = (type, registry) => (topic, schema, msg, parseOptions = null, schemaDependencies = []) => (() => {
  // For all schemas defined as dependencies, we have to parse them using avsc.
  // This will load them into an internal cache so when we parse the main schema the dependencies can be resolved
  if (schemaDependencies) {
    for (const schemaDependency of schemaDependencies) {
      avsc.parse(schemaDependency, parseOptions)
    }
  }
